...
class _SlidingUpPanelState extends State<SlidingUpPanel> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _ac = AnimationController(
      ...
      ..addListener(() {
        ...
        if (_ac.value == 1 && !_scrollingEnabled) {
          Future.delayed(const Duration(milliseconds: 100), () => setState(() => _scrollingEnabled = true));
        }

        if (_isPanelClosed && _scrollingEnabled) {
  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    return Listener(
      ...
      onPointerMove: (PointerMoveEvent p) {
        ...
        _onGestureSlide(p.delta.dy, direction: p.delta.direction);
      },
  ...
  void _onGestureSlide(double dy, {double direction = 0}) {
    ...
    if (_isPanelOpen && _sc.hasClients && _sc.offset <= 0 && direction > 0) {
      setState(() => _scrollingEnabled = false);
    }
  }
